...
<script>
		    	...
		    	if(this.multipleSelection.length==0){
			        ...
			        type: 'warning'
			      	...
			      	for(let i=0;i<multi.length;i++){
			      		delArr.push(multi[i].brandId);
			      	}
			      	let data={"brandId":delArr.join(',')};
			      	delBrand(this,data);
			      }).catch(() => {
			        this.$message({
			          type: 'info',
		    ...
		    submitForm(formName) {
		      	this.$refs[formName].validate((valid) => {
			        if (valid) {
			        	...
			        	this.$ajax.post(this.$store.state.localIP+'saveBrand',data)
			        	.then((response)=>{
			        		loading.close();
	...
	function brandList(obj){
		...
		obj.$ajax.post(obj.$store.state.localIP+"queryBrandList",{
	...
	function delBrand(obj,data){
		...
		obj.$ajax.post(obj.$store.state.localIP+"deleteBrand",data)
		.then(response=>{
			loading.close();
			//console.log(response)
			if(response.data.retMsg==0){
