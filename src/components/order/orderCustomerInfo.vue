<template>
	<div class="customerInfo">
		...
		<div class="customerInfoEdit">
			...
			<div class="submitOrder" v-if="isShowMeasureButton && !roomEdit">
				<el-button type="primary" @click="roomEdit=!roomEdit">量房</el-button>
			</div>
			<!--量房-->
			...
			<!--设计-->
			<el-card class="box-card" v-if='!isShowMeasureButton'>
				<div slot="header" class="clearfix">
					<span>设计图</span>
				</div>
				<div class="line"></div>
				...
				<el-row :gutter="20">
					<el-col :span="24" style="font-size:14px;color:#606266">
						效果图
					</el-col>
				</el-row>
				<div id="picShow">
		...
		<div class="submitOrder" v-if="nowStage=='5'?true:false">
			<el-button type="primary" @click="addOrderDialog">生成订单</el-button>
		</div>
		...
		<el-dialog 
			...
			<el-form :model="orderForm" label-width="80px">
				...
				<el-form-item label="性别">
					<el-select v-model="orderForm.sex" placeholder="性别">
						<el-option label="男" value="0"></el-option>
						<el-option label="女" value="1"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="联系电话">
					<el-input v-model="orderForm.mobile"  placeholder="联系电话"></el-input>
				</el-form-item>
			</el-form>
			<!--表单结束-->
...
export default{
	...
	//数据更新后
	updated(){
		if(!this.isShowMeasureButton && !this.DesignMapFlag){
			getImg('/ourHouse/static/img/1.jpg');
			this.DesignMapFlag=1
			/*window.onresize=()=>{
				getImg('/ourHouse/static/img/1.jpg');
			}*/
		};
	   ...
	   handleCommand2(command) {
	    ...
	    addOrder(){
	    	...
	    	let data={
	    		...
	    		/*custName:this.dataResults.name,
	    		mobileNum:this.dataResults.mobile,*/
	    		linkman:this.orderForm.name,
	    		linkmanSex:this.orderForm.sex,
	    		linkmanPhone:this.orderForm.mobile,
	    		/*province:this.ruleForm.province,
	    		city:this.ruleForm.city,
	    		cityID:this.ruleForm.cityID,
	    		district:this.ruleForm.district,
	    		buildingName:this.ruleForm.house,
	    		doorplate:this.ruleForm.home*/
	    	}
	    	this.$ajax.post(this.$store.state.localIP+"addOrder",data)
	    	.then((response)=>{
	    		if(response.status==200){
					...
					if(response.data.retCode==0){
						this.$message({
				       	...
				       	this.dialogVisible=false;
				       	this.$router.push({path:'/orderManage/orderCustomer'})
					}else{
		...
		.then((response)=>{
			...
			if(response.status==200){
				...
				if(dataList.retCode==0){
					...
					obj.orderForm.name=dataList.serviceInfo.custName || '';
					...
					obj.orderForm.mobile=dataList.serviceInfo.mobileNum || '';
					//房屋面积
...
/*3D全景图*/
function getImg(url){
	let div = document.getElementById('picShow');
