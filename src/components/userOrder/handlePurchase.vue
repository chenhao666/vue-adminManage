  ...
  <div class="userPrder">
    ...
    <el-card class="box-card">
      ...
      <el-tabs v-model="brandSelect" type="card" @tab-click="handleClick">
        <el-tab-pane v-for="(item,key) in goodsList" :key='key' :label="item.brandName" :name="String(item.brandId)"></el-tab-pane>
      </el-tabs>
      <!--采购列表-->
      <div class="box">
  ...
  export default {
    ...
    methods:{
      ...
      handleClick(tab, event) {
        ...
        var index=tab.index;
        this.brandSelect=String(this.goodsList[index].brandId);
    ...
    obj.$ajax.post(obj.$store.state.localIP+"queryPurchaseList",data)
      .then(response=>{
        ...
        if(response.data.retCode==0){
          ...
          obj.orderPrice=response.data.goodsOrder.totalAmout ? '￥'+response.data.goodsOrder.totalAmout : '';
          ...
          if(list.length>0){
            ...
            if(list.length>0){
              ...
              obj.brandSelect=String(list[obj.selectBrandIndex].brandId);
