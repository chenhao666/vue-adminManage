			...
			<div class="text item addPackageForm">
				...
				<div class="goodsList">
					<el-table
						...
						<el-table-column prop="typeName" :resizable="resizable"   label="位置" min-width="75">
						</el-table-column>
						<el-table-column prop="species" :resizable="resizable"   label="类型" min-width="75">
						</el-table-column>
						<el-table-column prop="title" :resizable="resizable"   label="图片" min-width="150">
						...
						<el-table-column prop="material" :resizable="resizable"   label="材质" min-width="100" show-overflow-tooltip>
						</el-table-column>
						<el-table-column prop="unitPrice" :resizable="resizable"   label="单价" min-width="75">
						</el-table-column>
						<el-table-column prop="goodsNum" :resizable="resizable"   label="数量">
						</el-table-column>
					</el-table>
				</div>
        <div class="priceSums">
          总计：{{goodsOrder.totalPrice?goodsOrder.totalPrice:totalAmout}}
				...
				<div style="text-align: center;padding: 30px 0px;">
					...
					<el-button type="primary" @click="submitForm('ruleForm')" v-if="editBtnShow">保存</el-button>
  ...
  computed:{
    totalAmout:function () {
      ...
      for(let i=0;i<this.editableTabs.length;i++){
        for (let j =0;j<this.editableTabs[i].goodsInfos.length;j++){
          if(this.editableTabs[i].goodsInfos){
            if(this.editableTabs[i].goodsInfos[j].species=='单品'||this.editableTabs[i].goodsInfos[j].species=='组合'){
              total = total + parseInt(this.editableTabs[i].goodsInfos[j].unitPrice * this.editableTabs[i].goodsInfos[j].goodsNum*100)
            }
          }
        }
      }
	...
	methods: {
		...
		handlePackageEdit(index, row){
			...
			.then(response=>{
				...
				if(list.length>0){
					this.tableData=sortData(list[0].goodsInfos);
	    ...
	    handleClose(done) {
      	...
      	goGroup(){
      		...
      		for(var i=0;i<list.length;i++){
      			if(list[i].species=="商品" || list[i].species=="替换"){
      				this.$message({
					    showClose: true,
					    message: '请选择单品进行组合！',
					    type: 'warning'
      		...
      		for(var i=0;i<list.length;i++){
      			if(packageIdArr.indexOf(parseInt(list[i].packageId))==-1){
      				for(var j=0;j<listAll.length;j++){
	      				if(listAll[j].species!="组合" && listAll[j].species!="替换" && listAll[j].packageId==list[i].packageId){
		...
		uploadSuccess(response, file, fileList){
	    	...
	    	if(len==num){
				...
				if(this.selectGroupNum==-1){
		    		...
		    		var list=this.multipleSelection;
					var listAll=this.tableData.concat([]);
					for(var i=0;i<list.length;i++){
						for(var j=0;j<listAll.length;j++){
							if(listAll[j].indexId==list[i].indexId){
								listAll[j].groupId=chirld.indexId;
								listAll[j].species='商品';
							}
						}
					}
			    ...
			    if (valid) {
					...
					if(this.editGoodsFloag==0){
						...
						if(this.selectGoodsType==0){
							...
							for(var i=0;i<listAll.length;i++){
								if(listAll[i].packageId==child.packageId){
									if(listAll[i].groupId || listAll[i].groupId==0){
										typeFlag.push(1);
										groupId=listAll[i].groupId;
										if(!flagName){
											flagName=listAll[i].typeName;
										}else{
											if(flagName!=listAll[i].typeName){
												flagName=1;
											}
										}
									}else if(listAll[i].species=="组合"){
										typeFlag.push(1);
									}else{
										typeFlag.push(0);
									}
						...
						for(let i=0;i<tabs.length;i++){
							if(tabs[i].packageId==child.packageId){
								emptyFlag=1;
								child.indexId=tabs[i].goodsInfos.length;
						...
						//console.log(this.tableData)
				   		...
				   		var packageObj={};
				   		for(var x in child){
				   			packageObj[x]=child[x];
				   		}
				   		var oldPackage=packageObj.packageId;
				   		var index="";
				   		for(var i=0;i<listAll.length;i++){
							if(listAll[i].indexId==selectList[0].indexId){
								index=i;
								var arr=this.addGoods.selectPackage.split(',');
								child.packageId=arr[0];
								child.packageName=arr[1];
								child.packageOrder=arr[2];
								if(this.addGoods.selectLocation){
									var selectLocationArr=this.addGoods.selectLocation.split(',');
									child.typeName=selectLocationArr[0];
									child.typeOrder=selectLocationArr[1]
						...
						if(parseInt(child.packageId)==parseInt(oldPackage)){
							...
							for(let i=0;i<tabs.length;i++){
						 		if(tabs[i].packageId==oldPackage){
						 			tabs[i].goodsInfos.splice(index,1);
						 			break;
						 		}
						 	}
							var emptyFlag=0;
...
function goodsList(obj){
	...
	.then(response=>{
		...
		if(list.length>0){
			obj.tableData=sortData(list[0].goodsInfos);
...
function goodsOrderInfo(obj,callback){
	...
	.then(response=>{
		...
		if(response.data.goodsOrder){
			var roleList=obj.$store.state.roleAuthList;
...
function sortData(list){
	...
	for(var i=1;i<arr.length;i++){
		if(arr[i].replaceId || arr[i].replaceId==0){
			if(arr[i-1].replaceId || arr[i-1].replaceId==0){
