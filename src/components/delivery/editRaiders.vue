<template>
	<div class="addRaiders">
		<el-breadcrumb separator-class="el-icon-arrow-right">
		  ...
		  <el-breadcrumb-item class="fontWeight">编辑攻略</el-breadcrumb-item>
		</el-breadcrumb>
		<div class="clear"></div>
...
//详细信息
function raidersInfo(obj){
	const loading =openLoad(obj);
	obj.$ajax.post(obj.$store.state.localIP+'queryDecorateInfoById',{id:obj.id})
	.then(response=>{
		//console.log(response);
		loading.close();
		if(response.data.retCode==0){
            let list=response.data.decorateinos;
            //标题
            obj.ruleForm.title=list.title;
			//封面
            obj.imageUrl=list.coverPic || "";
            if(list.coverPic){
            	obj.fileList=[{name:list.title,url:list.coverPic}];
            }
            //攻略类别
            if(list.stageId && list.stageName){
            	obj.ruleForm.raidersType=list.stageId+','+list.stageName;
            }
            //内容
            obj.editor.html(list.details || "");
        }else{
           	obj.$message.error(response.data.retMsg);
        }
	})
